...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768, num_frames=64, tubelet_size=2):
        super().__init__()
...
def get_videomaeModel(modelStyle="B"):

    """
    Function to return MAE model

    INPUTS:-
    1) modelStyle: Style of the model
    ...
    if(modelStyle == 'S'):
        ...
        path = './_store/weights/videomae_small_kinetics.pth'

    if(modelStyle == 'B'):
        model = vit_base_patch16_224()
        path = './_store/weights/videomae_base_kinetics.pth'

    if(modelStyle == 'L'):
        model = vit_large_patch16_224()
        path = './_store/weights/videomae_large_kinetics.pth'
        
    if(modelStyle == 'G'):
        model =  vit_huge_patch16_224()
        path = './_store/weights/videomae_huge_kinetics.pth'
...
class videomae(torch.nn.Module):
    ...
    def forward(self,x):
        ...
        x = x + self.pos_embed.to(x.device)[:,:x.size(1),:].clone().detach() # Positional Embedding, shape -> [b,thw,d]
        
        for blk in self.blocks:
            x = blk(x) # Tx output, shape -> [b,thw,d]
...
if __name__ == "__main__":
    ...
    a = torch.randn(size=(2,3,60,128,128)).to(device)
    model = videomae(143).to(device)
    op1 = model(a)
    print(op1.size())

    # parser = argparse.ArgumentParser()

    # parser.add_argument('--model_style',
    #                     type=str,
    #                     default='B')
    ...
    # model = videomaeBackbone(args).to(device)
    # b = model(a)
    # print(b.size())
    ...
    # path = './videomae_large_kinetics.pth'
    ...
    # pos_embed = model.pos_embed.to(device)

    # b = patch_embed(a)
    # print(b.size())

    # b = b + pos_embed[:,:b.size(1),:]
